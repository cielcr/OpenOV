...
bool selectOvbject = false;//用于标记是否有选取目标
...
void onMouse(int event, int x, int y, int,void*){
	...
	case CV_EVENT_LBUTTONDOWN:origin = cv::Point(x,y);
		seletion = cv::Rect(x,y,0,0);
		selectObject = true;
		break;
	case CV_EVENT_LBUTTONUP:selectOject = false;
...
//1.注册鼠标事件的毁掉函数，第三个参数是用户提供给回调函数的，也就是回调函数中最后的param参数
	...
	cv::Mat hsv,hue,mask,hisk,backproj;
	...
	if(trackOject < 0){
		...
		//将直方图归一
		calmalize(hist,hist,0,255,CV_MINMAX);
	...
	//调用Camshift 算法的接口
	cv::RotatedRTect trackBox = CamShift(backproj,trackWindow,cv::TermCriteria(CV_TERMCRIT_EPS | CV_TERMIT_ITER,10,1));
	...
	//如果正在选择追踪目标，则画出选择框
	if(selectObjetc && seletion.width > 0 && selection.height > 0){
